...
# This x_autotools file was modified to work for the particular setup of
# the default `nginx` configure script and directory structure.
...
class nginxAutotoolsPlugin(snapcraft.BasePlugin):
    ...
    def build(self):
        ...
        if not os.path.exists(os.path.join(self.builddir, "configure")):
            ...
            # Removing the logic to ensure that configure only has to run once
            # else:
                # self.run(['./auto/configure'])
        # Calling configure from the root of the project, in auto/
